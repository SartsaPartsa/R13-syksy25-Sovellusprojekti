import { getMovieDetails, getPopularMovies } from '../helper/tmdbClient.js'

// helper: map app locale to TMDB language
function toTmdbLang(lng = 'fi-FI') {
    return lng.startsWith('fi') ? 'fi-FI' : 'en-US'
}

// GET /api/movies/popular/today
export async function getPopularToday(req, res, next) {
    try {
        const language = toTmdbLang((req.query.language || 'fi-FI').trim())
        const data = await getPopularMovies(language, 1)
